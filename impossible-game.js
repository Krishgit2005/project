...
function positivMod(n, mod) {
  ...
  class drawing {
    ...
    drawGrid(gridInstance, heroInstance) {
      ...
      if (
        ...
        if (endingPosition != undefined) {
          this.drawTextNeonStyle("YOU WON!", 255, 215, 0, 1, 80, [
            endingPosition + 1 / 2,
            16,
          ]);
    ...
    drawScore(current, best) {
      // draw current score (top-right) and best score (below it)
      this.ctxBack.save();
  ...
  function restart(parameters) {
    ...
    // reset score starting reference when fully restarting to initial
  ...
  function deathFinish() {
    ...
    // start screen shake
    drawingInstance.shake.t = 14; // frames
  ...
  function winFinish() {
    ...
    if (
      ...
      // On win, show options: Enter/Space to restart, N for next level
      if (keys.Space || keys.Enter) {
        ...
        drawingInstance.ctx.fillText(
          "Press Enter to Restart • Press N for Next Level",
          drawingInstance.ctx.canvas.width / 2,
          drawingInstance.ctx.canvas.height / 2 + 60
        );
      }
    }
  }
  ...
  function game() {
    ...
    if (!heroInstance.isDead && !heroInstance.haveFinished) {
      ...
      // initialize startX on first frame after start
      if (!heroInstance.hasStarted) {
        startX = heroInstance.body.center.x;
      }
      heroInstance.move(gridInstance);
      // update score by distance traveled along x
  ...
  function level2(gridInstance, heroInstance) {
    /* A slightly harder layout: denser spikes and a longer run */
    let pos = [0, 4];
